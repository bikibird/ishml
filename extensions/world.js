...
ishml.reify.isDirectionOf=(operations,subjects,fore,aft)=> //garden is north of house -- item===house
{
	...
	subjects.forEach((subject,index)=>
	{
		...
		directObjects.forEach((directObject,index)=>
		{
			...
			{
				directObject.tie("exit:"+fore+"=exit:"+aft).to(subject)
			}
			else 
			{
				if (subject instanceof ishml.Ply)
				{
					var knot=subject.knot
					var direction=subject.id
					subject.untie()
					directObject.tie("exit:"+fore+"=exit:"+direction).to(knot)
					subject=knot.exit[direction]
				}
			}	
		})
...
ishml.reify.directionOf=(operations,fore,aft)=> 
//EAST OF garden is north of house
//EAST OF garden is house
{
	var operator=operations.next()
